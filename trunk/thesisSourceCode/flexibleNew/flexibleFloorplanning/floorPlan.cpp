...
void FloorPlan::readSoft(char *file){
  ...
  for(int i=0; !fs1.eof(); i++){
    // modules
    SoftModule dummy_softmod;
 
	fs1 >> dummy_softmod.initialAspectRatio;
	fs1 >> dummy_softmod.finalAspectRatio;
	fs1 >> dummy_softmod.incr;
	if (dummy_softmod.incr == 0) dummy_softmod.isSoft = false;
	else dummy_softmod.isSoft = true;
	softModules.push_back(dummy_softmod);	// new module
...
//    }
 ...
 for( i=0; i < softModules.size();i++){
	//cout << "Intial Aspect Ratio: " << softModules[i].initialAspectRatio<< endl;
    //cout << "Fianl Aspect Ratio: " << softModules[i].finalAspectRatio << endl;
    //cout << "Increment: " << softModules[i].incr << endl;
	cout << "Is Soft: " << softModules[i].isSoft << endl;
   //    cout << modules[i].pins.size() << " Pins:\n";
//    for(int j=0; j < modules[i].pins.size(); j++){
//      cout << query_map(net_table,modules[i].pins[j].net) << " ";
//      cout << modules[i].pins[j].x << " " << modules[i].pins[j].y << endl;
	...
	for (pass = 0; pass < n-1 && switched == true; pass++) {
		...
		for (j=0; j<n-pass-1; j++) {
			if (modules[j].area>modules[j+1].area) {
				...
				holdS = softModules[j] ;
				softModules[j] = softModules[j+1];
				softModules[j+1] = holdS;
			}
		}
	}	
}
...
void FloorPlan::list_information(){
 ...
 // printf( "Cost			  = %.6f\n", float(getCost()));
	...
	//void setChild(Modules &modules, int intialPosition, int childCount, bool isItLeaf, double initialAR, double finalAR, double softIncr, bool isItSoft);
	//	void setChildComposite(SuperModules &superModules, int intialPosition, int childCount, bool isItLeaf, double initialAR, double finalAR, double softIncr, bool isItSoft);
	
void SuperModule :: setChild(Modules &modules, int intialPosition, int childCount, bool isItLeaf, double initialAR, double finalAR, double softIncr, bool isItSoft){
  moduleCount = childCount;
  isLeaf = isItLeaf;
  childModule[0] = &modules[intialPosition];
 
  initialAspectRatio = initialAR;
  finalAspectRatio = finalAR;
  incr = softIncr;
  isSoft = isItSoft;
}

void SuperModule :: setChildComposite(SuperModules &superModules, int intialPosition, int childCount, bool isItLeaf, double initialAR, double finalAR, double softIncr, bool isItSoft){
  moduleCount = childCount;
  isLeaf = isItLeaf;
...
void SuperModule :: arrange4Blocks() {
	...
	if(((SuperModule *)childModule[0])->isSoft == false) {
		((SuperModule *)childModule[0])->initialAspectRatio = 0.0;
		((SuperModule *)childModule[0])->finalAspectRatio = 0.0;
		((SuperModule *)childModule[0])->incr = 0.1 ;

	}

	if(((SuperModule *)childModule[1])->isSoft == false) {
		((SuperModule *)childModule[1])->initialAspectRatio = 0.0;
		((SuperModule *)childModule[1])->finalAspectRatio = 0.0;
		((SuperModule *)childModule[1])->incr = 0.1 ;

	}
	if(((SuperModule *)childModule[2])->isSoft == false) {
		((SuperModule *)childModule[2])->initialAspectRatio = 0.0;
		((SuperModule *)childModule[2])->finalAspectRatio = 0.0;
		((SuperModule *)childModule[2])->incr = 0.1 ;

	}
	if(((SuperModule *)childModule[3])->isSoft == false) {
		((SuperModule *)childModule[3])->initialAspectRatio = 0.0;
		((SuperModule *)childModule[3])->finalAspectRatio = 0.0;
		((SuperModule *)childModule[3])->incr = 0.1 ;

	}

	for( double aRatio = ((SuperModule *)childModule[0])->initialAspectRatio; aRatio <=((SuperModule *)childModule[0])->finalAspectRatio;aRatio+=((SuperModule *)childModule[0])->incr) { 

		if(((SuperModule *)childModule[0])->isSoft == true ) {
			childModule[0]->height = sqrt(childModule[0]->area / aRatio);
			childModule[0]->width = sqrt(childModule[0]->area * aRatio);
		}

	for( double bRatio = ((SuperModule *)childModule[1])->initialAspectRatio; bRatio <=((SuperModule *)childModule[1])->finalAspectRatio;bRatio+=((SuperModule *)childModule[1])->incr) { 

		if(((SuperModule *)childModule[1])->isSoft == true ) {
			childModule[1]->height = sqrt(childModule[1]->area / bRatio);
			childModule[1]->width = sqrt(childModule[1]->area * bRatio);
		}
	for( double cRatio = ((SuperModule *)childModule[2])->initialAspectRatio; cRatio <=((SuperModule *)childModule[2])->finalAspectRatio;cRatio+=((SuperModule *)childModule[2])->incr) { 

		if(((SuperModule *)childModule[2])->isSoft == true ) {
			childModule[2]->height = sqrt(childModule[2]->area / cRatio);
			childModule[2]->width = sqrt(childModule[2]->area * cRatio);
		}

	for( double dRatio = ((SuperModule *)childModule[3])->initialAspectRatio; dRatio <=((SuperModule *)childModule[3])->finalAspectRatio;dRatio+=((SuperModule *)childModule[3])->incr) { 

		if(((SuperModule *)childModule[3])->isSoft == true ) {
			childModule[3]->height = sqrt(childModule[3]->area / dRatio);
			childModule[3]->width = sqrt(childModule[3]->area * dRatio);
		}

	//cout << aRatio << " " << bRatio << " " << cRatio << " " << dRatio << endl;
	for(int i=0; i < 24;i++) {
		for(int j=0; j < 16;j++) {
			for(int l=0; l < 4; l++){
	...
	if(((SuperModule *)childModule[0])->isSoft == true ) {
		childModule[0]->height = sqrt(childModule[0]->area / tempARatio);
		childModule[0]->width = sqrt(childModule[0]->area * tempARatio);
	}
	if(((SuperModule *)childModule[1])->isSoft == true ) {
		childModule[1]->height = sqrt(childModule[1]->area / tempBRatio);
		childModule[1]->width = sqrt(childModule[1]->area * tempBRatio);
	}
	if(((SuperModule *)childModule[2])->isSoft == true ) {
		childModule[2]->height = sqrt(childModule[2]->area / tempCRatio);
		childModule[2]->width = sqrt(childModule[2]->area * tempCRatio);
	}
	if(((SuperModule *)childModule[3])->isSoft == true ) {
		childModule[3]->height = sqrt(childModule[3]->area / tempDRatio);
		childModule[3]->width = sqrt(childModule[3]->area * tempDRatio);
	}
...
void SuperModule :: arrange3Blocks() {
	...
	//cout << aRatio << " " << bRatio << " " << cRatio << endl;
...
//	}
	...
	/*if (!((SuperModule *)childModule[0])->isLeaf) {
		if (isSoft) {
			initialAspectRatio = ((SuperModule *)childModule[0])->initialAspectRatio;
			finalAspectRatio = ((SuperModule *)childModule[0])->finalAspectRatio;
			incr = ((SuperModule *)childModule[0])->incr;
		} else {
			initialAspectRatio = 0.0;
			finalAspectRatio = 0.0;
			incr = 0.0;
		}
...
void SuperModule::set1ChildCordinates() {
	...
	if (((SuperModule *)childModule[0])->isSoft) {
		childModule[0]->height = height;
		childModule[0]->width = width;
	}
}
