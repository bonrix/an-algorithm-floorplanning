...
/*	for(int i=0; i < fp.modules.size();i++){
	...
	for(int i=0; i < fp.modules.size();i++){
		...
		if (fp.softModules[i].incr == 0) 
			dummy_mod.setChild(fp.modules,i,1,true,fp.softModules[i].initialAspectRatio,fp.softModules[i].finalAspectRatio,fp.softModules[i].incr,false);
		else
			dummy_mod.setChild(fp.modules,i,1,true,fp.softModules[i].initialAspectRatio,fp.softModules[i].finalAspectRatio,fp.softModules[i].incr,true);
...
void recursiveBottomUp(SuperModules &superModules) {
	...
	/*for(int i=0; i < superModules.size();i++){
		if((i % 4) == 0) {
			...
			if( i+4 > superModules.size()) {
				if(superModules.size()%4 == 0 ) {
					dummy_mod.setChildComposite(superModules,i,superModules.size()%4,false,superModules[i].initialAspectRatio, superModules[i].finalAspectRatio, superModules[i].incr, superModules[i].isSoft);
				} else {
					dummy_mod.setChildComposite(superModules,i,superModules.size()%4,false, 0.0, 0.0, 0.0, false);
				}
			} else {
				dummy_mod.setChildComposite(superModules,i,4,false, 0.0, 0.0, 0.0, false);
			}
	...
	for(int i=0; i < superModules.size();i++){
		if((i % 4) == 0) {
			SuperModule dummy_mod;
			if( i+4 > superModules.size()) {
				if(superModules.size()%4 == 1 ) {
					dummy_mod.setChildComposite(superModules,i,1,false,superModules[i].initialAspectRatio, superModules[i].finalAspectRatio, superModules[i].incr, superModules[i].isSoft);
				} else {
					dummy_mod.setChildComposite(superModules,i,superModules.size()%4,false, 0.0, 0.0, 0.0, false);
				}
			} else {
